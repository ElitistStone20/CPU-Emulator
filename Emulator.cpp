...
void Group_1(BYTE opcode){
	...
	switch(opcode) {
		//LOAD OPCODES
		case 0x43: //Load Immidiate
			data = fetch();
			Registers[REGISTER_A] = data;
			break;
		case 0x53:    //Load Absolute
			LB = fetch();
			HB = fetch();
			address += (WORD)((WORD)HB << 8) + LB;
			if (address >= 0 && address < MEMORY_SIZE) { Registers[REGISTER_A] = Memory[address]; }
			break;
		case 0x63:   //Load absolute X
		...
		case 0x83:	//Load absolute X,Y
			...
			if (address >= 0 && address < MEMORY_SIZE) { Registers[REGISTER_A] = Memory[address]; }
			break;
		case 0x93:    //Load Zero page 
			address += 0x0000 | (WORD)fetch();
			if (address >= 0 && address < MEMORY_SIZE) { Registers[REGISTER_A] = Memory[address]; }
			break;
		//STORE OPCODES
		case 0x04:  //Store abolsute
			LB = fetch();
			HB = fetch();
			address += (WORD)((WORD)HB << 8) + LB;
			if (address >= 0 && address < MEMORY_SIZE) { Memory[address] = Registers[REGISTER_A]; }
			break;
		case 0x14:	//Store absolute X
			address += Index_Registers[REGISTER_X];
			LB = fetch();
			HB = fetch();
			address += (WORD)((WORD)HB << 8) + LB;
		...
		case 0x34:	//Store absolute X,Y
			...
			if (address >= 0 && address < MEMORY_SIZE) { Memory[address] = Registers[REGISTER_A]; }
			break;
		case 0x44:	//Store Zero page
			address += 0x0000 | (WORD)fetch();
			if (address >= 0 && address < MEMORY_SIZE) { Memory[address] = Registers[REGISTER_A]; }
			break;
		//MVR Opcodes (More needed)
		case 0x27:	
			data = fetch();
			Registers[REGISTER_B] = data;
			break;
		//LODS opcodes below (More needed)
		case 0x20:	//LODS #
			data = fetch();
			StackPointer = data; StackPointer += (WORD)fetch() << 8;
			break;
		case 0x30:	//LODS absolute
			LB = fetch();
			HB = fetch();
			address += (WORD)((WORD)HB << 8) + LB;
		...
		case 0x70:	//LODS Zero page
			...
			break;
		case 0xB6:    //Add with carry A,B
			temp_word = (WORD)Registers[REGISTER_A] + (WORD)Registers[REGISTER_B];
			if ((Flags & FLAG_C) != 0) { temp_word++; }
			if (temp_word >= 0x100) { Flags = Flags | FLAG_C; }  //Sets FLAG_C
			else { Flags = Flags & (0xFF - FLAG_C); }			 //Clears FLAG_C
			set_flag_n((BYTE)temp_word);
			set_flag_z((BYTE)temp_word);
			Registers[REGISTER_A] = (BYTE)temp_word;
			break;
		//CAMP functions below (Add more)
		case 0xBA:	//CAMP B,A
			temp_word = (WORD)Registers[REGISTER_A] - (WORD)Registers[REGISTER_B];
			if (temp_word >= 0x100) {
...
void Group_2_Move(BYTE opcode){
	...
	int destReg = 0, sourceReg = 0;
	switch(destination) {
	case 0x00:
		destReg = REGISTER_A;
		break;
	case 0x01:
		destReg = REGISTER_B;	//Complete the rest of this
		break;
	}
	switch (source) {
	case 0:
	...
	else if (sourceReg == REGISTER_M) {	//Need a else if for if both DestReg and SourceReg is REGISTER_M
